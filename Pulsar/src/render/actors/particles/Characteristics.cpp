...
namespace Particles {
	...
	CharacteristicGen Characterize(CHRFunc&& f, DataIndex max_index)
	{
		...
		return make_functor<false>([](const CHRSeed&, const cls& tuple) -> CHRBind {
			return { make_functor<false>([](Particle& p, const CHRFunc& f) -> void {
				f(p);
			}, std::get<0>(tuple)), std::get<1>(tuple) };
		}, cls(std::move(f), max_index));
	}
	
	CharacteristicGen Characterize(Functor<void, const std::tuple<Particle&, const CHRSeed&>&>&& f, DataIndex max_index)
	{
		using tpl = std::tuple<Particle&, const CHRSeed&>;
		using cls = std::tuple<Functor<void, const tpl&>, DataIndex>;
		return make_functor<false>([](const CHRSeed& seed, const cls& tuple) -> CHRBind {
			using cls_inner = std::tuple<Functor<void, const tpl&>, const CHRSeed&>;
			return { make_functor<false>([](Particle& p, const cls_inner& tuple) -> void {
	...
	{
		return make_functor<false>([](const CHRSeed& seed, const std::vector<CharacteristicGen>& characteristics) -> CHRBind {
			...
			return { make_functor<false>([](Particle& p, const std::vector<CHRFunc>& funcs) -> void {
				for (auto& f : funcs)
					f(p);
	...
	{
		return make_functor<false>([](const CHRSeed& seed, const CharacteristicGen& setup) -> CHRBind {
	...
	CharacteristicGen CombineInitialOverTime(CharacteristicGen&& initial, CharacteristicGen&& over_time)
	{
		...
		return make_functor<false>([](const CHRSeed& seed, const cls& pair) -> CHRBind {
			CHRBind initial(pair.first(seed));
			CHRBind over_time(pair.second(seed));
			using cls_inner = std::pair<CHRFunc, CHRFunc>;
			return { make_functor<false>([](Particle& p, const cls_inner& pair) -> void {
				if (p.t() == 0.0f)
					pair.first(p);
				else
					pair.second(p);
	...
	namespace CHR {
		...
		{
			return Characterize(make_functor<true>([](Particle& p, DataIndex si) -> void {
				p[si] = rng();
			}, si), si + 1);
		}
		...
		CharacteristicGen FeedData(DataIndex si, float v)
		{
			...
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
		...
		{
			return make_functor<true>([](const CHRSeed& seed, DataIndex si) -> CHRBind {
		...
		CharacteristicGen OperateData(DataIndex si, Functor<void, float&>&& func)
		{
			using cls = std::tuple<DataIndex, Functor<void, float&>>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
				// func(p[si]);
		...
		CharacteristicGen OperateData(DataIndex si, Functor<void, const std::tuple<float&, float>&>&& func, DataIndex di)
		{
			using cls = std::tuple<DataIndex, Functor<void, const std::tuple<float&, float>&>, DataIndex>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
				// func({ p[si], p[di] });
		...
		CharacteristicGen OperateDataUsingSeed(DataIndex si, Functor<void, const std::tuple<float&, const CHRSeed&>&>&& func)
		{
			using cls = std::tuple<Functor<void, const std::tuple<float&, const CHRSeed&>&>, DataIndex>;
			return Characterize(make_functor<false>([](const std::tuple<Particle&, const CHRSeed&>& tuple, const cls& c) -> void {
				// func({ p[si], seed });
				std::get<0>(c)({ std::get<0>(tuple)[std::get<1>(c)], std::get<1>(tuple) });
			}, cls(std::move(func), si)), si + 1);
		}

		CharacteristicGen SetColorUsingData(Functor<Modulate, float>&& color, DataIndex di)
		{
			using cls = std::tuple<DataIndex, Functor<glm::vec4, float>>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
		...
		CharacteristicGen SetColorUsingTime(Functor<Modulate, float>&& color)
		{
			using cls = Functor<glm::vec4, float>;
			return Characterize(make_functor<false>([](Particle& p, const cls& color) -> void {
				*p.m_Shape->Fickler().Modulate() = color(p.t());
			}, std::move(color)), 0);
		}

		CharacteristicGen SetLocalScaleUsingData(Functor<Scale2D, float>&& scale, DataIndex di)
		{
			using cls = std::tuple<Functor<glm::vec2, float>, DataIndex>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
		...
		{
			return Characterize(make_functor<false>([](Particle& p, const Position2D& position) -> void {
				*p.m_Shape->Fickler().Position() = position;
			}, position), 0);
		}

		CharacteristicGen SetLocalPositionUsingData(Functor<Position2D, float>&& position, DataIndex di)
		{
			using cls = std::tuple<Functor<glm::vec2, float>, DataIndex>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
		...
		CharacteristicGen SetLocalPositionUsingData(Functor<Position2D, const glm::vec2&>&& position, DataIndex dix, DataIndex diy)
		{
			using cls = std::tuple<Functor<glm::vec2, const glm::vec2&>, DataIndex, DataIndex>;
			return Characterize(make_functor<false>([](Particle& p, const cls& tuple) -> void {
		...
		static CharacteristicGen _SyncAll = Characterize([](Particle& p) { p.m_Shape->Fickler().SyncAll(); }, 0);
		static CharacteristicGen _SyncT = Characterize([](Particle& p) { p.m_Shape->Fickler().SyncT(); }, 0);
		static CharacteristicGen _SyncP = Characterize([](Particle& p) { p.m_Shape->Fickler().SyncP(); }, 0);
		static CharacteristicGen _SyncRS = Characterize([](Particle& p) { p.m_Shape->Fickler().SyncRS(); }, 0);
		static CharacteristicGen _SyncM = Characterize([](Particle& p) { p.m_Shape->Fickler().SyncM(); }, 0);
		...
		Functor<float, const CHRSeed&> Seed_waveT()
		{
			return [](const Particles::CHRSeed& seed) { return seed.waveT; };
		}
