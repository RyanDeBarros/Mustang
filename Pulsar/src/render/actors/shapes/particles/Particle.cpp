...
Particle::Particle(const std::shared_ptr<DebugPolygon>& shape, const LocalTransformer2D& transformer, const float& lifespan, const Particles::CHRBind& characteristic)
	: m_Shape(shape), m_Transformer(transformer), m_Characteristic(characteristic.first), m_LifespanInv(1.0f / lifespan), m_Data(characteristic.second)
{
	m_Characteristic(*this);
}

Particle::Particle(Particle&& other) noexcept
	: m_Shape(other.m_Shape), m_Transformer(other.m_Transformer), m_Characteristic(other.m_Characteristic), m_LifespanInv(other.m_LifespanInv), m_Data(other.m_Data), m_Invalid(other.m_Invalid), m_T(other.m_T), m_DT(other.m_DT)
{
	m_Characteristic(*this);
}
