...
template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::Insert(const _KeyFrame& key_frame)
{
	insert_sorted(keyFrames, key_frame, comparator);
}

template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::Insert(_KeyFrame&& key_frame)
{
	insert_sorted(keyFrames, std::move(key_frame), comparator);
}

template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::SetOrInsert(const _KeyFrame& key_frame)
{
	set_or_insert_sorted(keyFrames, key_frame, equalitor, comparator);
}

template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::SetOrInsert(_KeyFrame&& key_frame)
{
	set_or_insert_sorted(keyFrames, std::move(key_frame), equalitor, comparator);
}

template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::AdvanceForward(float time, float period)
{
	if (keyFrames.empty())
		return;
...
template<typename _Target, std::derived_from<KeyFrame<_Target>> _KeyFrame, unsigned int _InterpMethod>
void AnimationTrack<_Target, _KeyFrame, _InterpMethod>::AdvanceBackward(float time, float period)
{
	if (keyFrames.empty())
		return;
	if constexpr (_KeyFrame::progressive)
