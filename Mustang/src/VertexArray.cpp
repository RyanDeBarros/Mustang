...
static void inline setup_vertices(GLfloat vertices[], BufferCounter& num_vertices, unsigned short& layout_mask, unsigned int& layout)
{
...
static void inline setup_indices(GLuint indices[], BufferCounter num_indices)
{
	TRY(glBufferData(GL_ELEMENT_ARRAY_BUFFER, num_indices * sizeof(GLuint), indices, GL_STATIC_DRAW));
}

VertexArray::VertexArray(GLfloat vertices[], BufferCounter num_vertices, VertexLayoutMask layout_mask, VertexLayout layout, GLuint indices[], BufferCounter num_indices)
